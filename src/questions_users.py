"""module questions user about input"""
import os
...
class QuestionsUsers:
    ...
    def question_matrix_dimensions(self) -> list:
        """Function to questions about dimensions"""
        while True:
            dimensions_matrix = input("\nIndicate labyrinth dimensions labyrinth (x, y): \n")
            msg_dimensions_error = "\n\nIncorrect dimensions, please enter values properly, " \
                                   "numeric values" \
            ...
            try:
                ...
                try:
                    ...
                    print("Not constrains "
                          "accomplished. " + msg_dimensions_error)
                except ValueError as err:
                    self.logger.warning(f"[question matrix dimensions] "
                                        f"not numeric value, value error: {err}")
                    print("Not numeric value. " + msg_dimensions_error)
            except ValueError as err:
                self.logger.warning(f"[question matrix dimensions] not numeric value,"
                                    f" value error: {err}")
                print("Not numeric value. " + msg_dimensions_error)
            except IndexError as err:
                self.logger.warning(f"[question matrix dimensions] not indicated "
                                    f"dimensions properly, "
                                    f"index error: {err}")
                print("Not indicated dimensions properly " + msg_dimensions_error)

    @Decorators.decorator_write_log_init_end_method
    def question_probability_obstacles(self) -> float:
        """Function to questions about probability of obstacles"""
        while True:
            probability_find_obstacle_input = input("\nIndicate probability find obstacle "
                                                    "[numeric value amongst 0 - 1]: \n")
            msg_probability_error = "\n\nIncorrect probability, value error, " \
                                    "please enter values properly [value " \
                                    "amongst " \
                                    "0 - 1] "
            try:
                ...
                self.logger.warning(f"[question probability obstacles] "
                                    f"not indicated probability properly, "
                                    f"value error: {err}")
                print(msg_probability_error)

    @Decorators.decorator_write_log_init_end_method
    def question_symbol_corridors(self) -> str:
        """Function to questions about space symbols on labyrinth"""
        while True:
            agree_corridors_symbol = input("Corridors are represents with '.', "
                                           "are you agree [Y, N]: \n")
            if agree_corridors_symbol == "Y":
                return "."
            elif agree_corridors_symbol == "N":
                base_element_not_obstacle = input("Please enter corridor symbol to "
                                                  "use in labyrinth: \n")
    ...
    def question_symbol_obstacles(self) -> str:
        """Function to questions about obstacles symbols"""
        while True:
            agree_obstacles_symbol = input("Obstacles are represents with '#', "
                                           "are you agree [Y, N]: \n")
            if agree_obstacles_symbol == "Y":
                return "#"
            elif agree_obstacles_symbol == "N":
                base_element_obstacle = input("Please enter corridor symbol "
                                              "to use in labyrinth: \n")
                return base_element_obstacle
            else:
                self.logger.warning(f"[question symbol obstacles] incorrect "
                                    f"question option, "
    ...
    def initial_question_user_method_input_labyrinth(self) -> int:
        """Function to questions about initial labyrinth input"""
        while True:
            option_chosen = input("\n\nInit labyrinth (select an option):"
                                  "\n- 1. from file \n- 2. directly here"
                                  "\n- 3. automatic generation\n\nChosen option:\n")
            msg_error = "\n\nIncorrect option, please enter only numeric values, " \
                        "[1, 2 or 3, amongst options]"
            try:
                ...
                self.logger.warning(f"[initial question user method input labyrinth]"
                                    f" incorrect question option, "
                                    f"error : {err}")
                print(msg_error)

    @Decorators.decorator_write_log_init_end_method
    def question_file_name(self) -> str:
        """Function to questions about file name with labyrinth input"""
        while True:
            file_name = input("\n\nIndicate file name [file has to be in 'input' folder,"
                              " txt format, see example,"
                              "'example_labyrinth.txt': \n")
            msg_error = "\n\nFile not found, indicate properly file name in" \
                        " input folder with txt format"
            try:
    ...
    def question_symbols_on_file_or_list(self,
                                         format_input) -> tuple:
        """Function to questions symbols when input it is direct list or file"""
    ...
    def question_direct_list(self) -> list:
        """Function to questions about direct list input labyrinth"""
        while True:
            ...
            try:
                ...
                dimension_x = len(
                    [element.replace("]", "") for element in matrix.split("[")
                     if len(element) >= 1][0].split(",")) - 1

                matrix = [element.split(",") for element in [element.replace("]", "")
                                                             for element in matrix.split("[")
                                                             if len(element) >= 1]
                          if element.split(",") != '  ']
                ...
                for element_in_y_dimension in range(dimension_y):
                    ...
                    for element_in_x_dimension in range(dimension_x):
                        row.append(str(matrix[element_in_y_dimension]
                                       [element_in_x_dimension].replace("'",
                                                                        "").strip()))
